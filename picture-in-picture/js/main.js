    ...
    function handleFileComplete() {
	    ...
	    start.addEventListener("touchstart",function(){
		    timer = setInterval(function () {
			    ...
			    // 核心代码 start
			    context.drawImage(img,
				    (current_img.areaL) + (0 - current_img.areaL) * rate,
				    (current_img.areaT) + (0 - current_img.areaT) * rate,
				    (current_img.areaW) + ( img.width - current_img.areaW ) * rate,
				    (current_img.areaH) + ( img.height - current_img.areaH ) * rate,
				    0, 0, w, h);
			    // 趋近下一张图，且不是最后一张时，画出下一张图
			    if (rate < 0.3 && img_num > 0) {
				    // 750/裁切图片的宽度=缩放的比例
				    // 750/( (current_img.areaW) + (img.width - current_img.areaW) * rate )
				    context.drawImage(img2,
					    ( current_img.areaL * rate )*( 750 /( (current_img.areaW) + (img.width  - current_img.areaW) * rate ) ),
					    ( current_img.areaT * rate )*( 1206/( (current_img.areaH) + (img.height - current_img.areaH) * rate ) ),
					    ( current_img.areaW )*( 750/( (current_img.areaW) + (img.width - current_img.areaW) * rate ) ),
					    ( current_img.areaH )*( 1206/( (current_img.areaH) + (img.height - current_img.areaH) * rate ) )
				    );
			    }
			    // 核心代码 end
			    newStep--;
